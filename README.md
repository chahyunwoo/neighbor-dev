...
| `.env.local` | API 키·SMTP | `.env.example` 을 보고 새로 채운다 |
...
그 뒤 **2026-09-16 실측에서 `3100`·`3101` 도 다른 저장소의 dev 서버가 잡고 있었다.**

**이 저장소는 web `21200` · api `21201` 을 쓴다(전역 포트 블록 `21200`). DB 는 두지 않는다.**
남의 프로세스·터널을 끄지 않는다 — 겹치면 우리가 비킨다.
