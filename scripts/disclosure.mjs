...
const CHECKS = [
  ...
  /*
   * 클라이언트 시스템의 **API 엔드포인트 경로** (이슈 #86).
   *
   * 🔴 왜 잡는가: 기존 검사 축은 사명·개인 도메인·저장소명·사설 IP·시크릿이다.
   *    **클라이언트 시스템 내부를 가리키는 식별자를 보는 축이 없었고**, 실제로
   *    사례 상세 1건의 `decisions` 에 버전 붙은 엔드포인트 경로가 그대로 나가
   *    배포됐다. 검사기는 그동안 초록이었다.
   *
   *    AGENTS.md 「공개 금지」 5번 — 기술 과제는 쓰되 그 위에 얹힌 업무 규칙은
   *    쓰지 않는다. 엔드포인트 경로는 "무엇을 만들었나"가 아니라 **남의 시스템
   *    구조**다.
   *
   * ⚠️ **버전 세그먼트를 요구한다**(`/v1/`·`/v2/`). 그냥 `/[a-z]+/` 로 잡으면
   *    문서 경로(`/work/claude-board`)·파일 경로(`/src/app`)가 전부 걸려
   *    사람이 이 검사를 끈다. 실제 유출 형태가 버전 붙은 REST 경로였다.
   *
   * ⚠️ 이 저장소 **자신의** API 는 버전 세그먼트를 쓰지 않는다
   *    (`apps/api` 는 `@Controller('contact')`·`@Controller('diagnose')`).
   *    그래서 자기 경로가 오탐으로 걸리지 않는다 — 우연이 아니라 확인한 것이다.
   */
  ['클라이언트API경로', (t) => [...t.matchAll(/\/v\d+\/[a-zA-Z][\w/-]*/g)].map((m) => m[0])],
  /*
   * 클라이언트 **서버 응답 스펙의 키 나열** (이슈 #86).
   *
   * `{key, label, type, options, range, required}` 처럼 남의 API 가 내려주는
   * 필드 이름을 통째로 옮겨 적은 형태. 스펙 자체가 클라이언트 자산이다.
   *
   * ⚠️ **셋 이상**을 요구한다. 둘짜리(`{server,browser}`·`{x, y}`)는 설명문에
   *    흔히 쓰이는 표기라 오탐이 쏟아진다 — 실측으로 정한 경계다.
   */
  [
    '응답스펙키나열',
