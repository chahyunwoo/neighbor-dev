...
/*
 * 익명 카드 본문 — **`ANONYMOUS_LABELS` 와 같은 이유로 여기서 정한다**(#84).
 *
 * 🔴 정본의 `cardBody` 보다 **위**에 둔다. 라벨과 같은 자리다 — 카드 본문도
 *    사이트에 그대로 나가는 자유 텍스트라, 정본이 쓴 문장이 익명 방어선을
 *    우회할 길을 만들면 안 된다.
 *
 * ⚠️ 이 4건은 개인 사이트라 정본의 문장에 도메인·스코프 패키지명이 섞여 있다.
 *    `cafe24-playbook` 은 경력 요약 층이라 카드 본문 자체가 나가지 않아 없다.
 */
const ANONYMOUS_CARD_BODIES = {
  'hyunwoo-dev-admin':
    '글을 쓰고 고치는 화면은 로그인 뒤에서만 돌아서, 검색 노출보다 쓰던 내용을 잃지 않는 것이 중요했다. 편집 화면을 따로 두고 갈라져 있던 화면 부품을 한 벌로 합쳤다.',
  'hyunwoo-dev-blog':
    '글이 저장소 안 파일이라 오타 하나 고치는 데도 배포가 통째로 돌았다. 글쓰기와 배포를 떼어내되, 무엇을 고치면 어디가 새로 보이는지가 코드에 남게 했다.',
  'hyunwoo-dev-monorepo':
    '사이트 셋이 따로 놀면서 같은 것을 세 번 만들고 있었다. 한 저장소로 묶어 타입과 화면 요소를 한 소스에서 나눠 쓰되, 빌드가 서버 사정에 끌려가지 않게 했다.',
  'hyunwoo-dev-web':
    '직접 수주하는 입장에서는 무엇을 만들 수 있는지를 문장이 아니라 화면 자체로 보여줘야 했다. 움직임을 넣되 그 비용이 첫 로딩과 저사양 기기에서 감당되는 선에 머물게 하는 것이 과제였다.',
}

/**
 * 이 항목의 카드 본문을 사이트가 정하는가. 정하면 그 문자열, 아니면 `undefined`.
 * `anonymousLabelFor` 와 짝이다 — 호출부가 `??` 로 정본 값과 잇는다.
 */
export function anonymousCardBodyFor(id) {
  return ANONYMOUS_CARD_BODIES[id]
}
