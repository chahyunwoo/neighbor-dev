...
/**
 * 물건별 초점 보정 — 카메라가 바라볼 지점을 물건 위치에서 얼마나 옮길지.
 *
 * 🔴 **벽에 붙은 물건이 문제다.** 초점을 물건 그대로 두면 카메라가 그 물건과
 *    방 중심을 잇는 선 위에 서는데, 벽에 붙은 물건은 그 선이 **벽을 향한다** —
 *    현관문(x=-2.89, 왼쪽 벽)을 열면 문이 화면을 덮고 방이 안 보였다
 *    (2026-09-10 스크린샷으로 확인. 마커 7개 중 6개가 화면 밖으로 밀려
 *     가장자리에 접혔다).
 *
 * ⚠️ 값은 눈대중이 아니다(CLAUDE.md). 방 중심 방향으로 얼마나 당길지를
 *    **후보를 렌더해 스크린샷으로 비교**해 정한다. 아래 주석에 근거를 남긴다.
 */
export const FOCUS_PULL: Record<string, number> = {
  /*
   * 현관문 — 방 중심 쪽으로 당긴다.
   * 실측(2026-09-10) — 후보를 렌더해 스크린샷으로 비교했다:
   *   0    문이 화면을 덮고 방이 안 보인다(마커 6/7 이 화면 밖으로 밀렸다)
   *   0.8  여전히 어둡다(6/7)
   *   1.6  ✅ 문이 화면 중앙에 서고 방 맥락도 남는다(5/7)
   *   2.4  방은 잘 보이나 문이 오른쪽 가장자리로 밀려 주인공이 아니게 된다
   */
  door: 1.6,
}
